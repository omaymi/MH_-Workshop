...
export async function optimizeScheduleLocally(
    ...
    // Use a timeout to allow the UI to render the loading state before blocking the main thread
    ...
    console.log('GA Finished. Best Fitness:', gaSol.fitness);

    console.log('Starting Tabu Search...');
    const finalSol = runTabuSearch(gaSol, coursesList, slotIds, roomsDict, coursesDict, roomIds);
    console.log('Tabu Finished. Final Fitness:', finalSol.fitness);

    const timestamp = new Date().toLocaleString();

    // Return format matching the App's expectation
    return {
        name: `Schedule (Local) ${timestamp}`,
